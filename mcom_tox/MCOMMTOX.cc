...
int MCOMMTOX::start ()
{
    ...
    if (errNew != TOX_ERR_NEW_OK)
    {
        ...
        if (errNew == TOX_ERR_NEW_LOAD_BAD_FORMAT)
            pszErrcode = "Bad save format";
        else
            pszErrcode = "Unknown";
    ...
    tox_self_set_name (tox_, (uint8_t *)strName.c_str (), strName.size (),
                       NULL);
    ...
    tox_bootstrap (
        tox_, Dictionary_get (dictConfig_, "Tox.BootstrapIP"),
...
void MCOMMTOX::acceptFriendRequest_ (const char * pszAddress)
{
    ...
    if (ferr == TOX_ERR_FRIEND_ADD_ALREADY_SENT)
        dbg ("already\n") else if (ferr == TOX_ERR_FRIEND_ADD_BAD_CHECKSUM)
            dbg ("bad checksum\n") else
        {
...
void MCOMMTOX::addFriend_ (const char * addr, const char * msg)
{
    ...
    mtx_lock (&mtxTox_);
    dwNewNum =
        tox_friend_add (tox_, bID, (uint8_t *)msg, strlen (msg), &errFriend);
    mtx_unlock (&mtxTox_);
