...
#define FLCB(FUNC)                                                             \
    reinterpret_cast< FLTK_Callback_f > (                                      \
        flThunk< GFLTransfer, decltype (&GFLTransfer::FUNC),                   \
                 &GFLTransfer::FUNC >),                                        \
        this

GFLTransfer::GFLTransfer (class GUIFLTK * gui, XWContact_t * pcontact,
                          const char * filename, struct tm * time,
                          unsigned int num, unsigned int size, unsigned int dir)
    : contact_ (pcontact), gui_ (gui), filename_ (strdup (filename)),
