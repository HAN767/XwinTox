...
void ModCellCallback(MyTable *w, void* rowdata, void* userdata)
{
	...
	List_t *lstClasses =static_cast<List_t*>
	                    (self->hObj_->pSvcs->fnCall(self->hObj_,
	                            "/GETMODULECLASSESINFO", rowdata));
...
void GSettings::resize(int X, int Y, int W, int H)
{
	...
	tblClasses->resize(ppX, ppY + (ppH / 2 - (10 * scale)), ppW, ppH / 2 - (20 * scale));
