...
void sb_post (int mtype, PBMessage_t * msg, void * custom)
{
    ...
    if (me->frs > 1)
        sprintf (newlabel, "%d Friend\n Requests", me->frs);
    else if (me->frs == 1)
        sprintf (newlabel, "1 Friend\nRequest");
    else
        sprintf (newlabel, "0 Friend\n Requests");
...
void Sidebar::resize (int X, int Y, int W, int H)
{
    ...
    frbutton->resize (X + W - (72 * scale), y () + (64 * scale), 60 * scale,
                      30 * scale);

    f_reqs->resize (X + 3 * scale, Y + (96) * scale, W - (15 * scale),
                    frheight * scale);
