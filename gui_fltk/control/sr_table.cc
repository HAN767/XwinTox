    ...
    case CONTEXT_COL_HEADER:
        fl_push_clip (X, Y, W, H);
        {
            ...
            if (C < 9)
            {
                ...
                fl_color (FL_BLACK);
                fl_draw (G_header[C], X + 2, Y, W, H, FL_ALIGN_LEFT, 0,
                         0); // +2=pad left
...
void MyTable::autowidth (int pad)
{
    ...
    for (int r = 0; r < (int)_rowdata.size (); r++)
    {
        ...
        {
            w = 0;
            fl_measure (_rowdata[r].cols[c], w, h,
                        0); // pixel width of row text
...
void MyTable::load_command (void * userdata,
...
void MyTable::event_callback2 ()
{
    ...
    case CONTEXT_CELL:
    {
        if (Fl::event () == FL_PUSH && Fl::event_button () == 1)
        {
            ...
            cellcallback (this, _rowdata[rownum].userdata,
                          userdatacellcallback);
        }
    }
