...
void mareaFrMsg (int, PBMessage_t * msg, void * custom)
{
    ...
    self->moutput->scroll (
        self->moutput->count_lines (0, self->moutbuffer->length (), 1), 0);
    self->redraw ();
    Fl::unlock ();
    Fl::awake ();
}
...
void mareaSendPressed (Fl_Widget * B, void *)
{
    ...
    if (!((GMessageArea *)B->parent ())->mtype) /* contact */
    {
        ...
        cArea->moutput->scroll (
            cArea->moutput->count_lines (0, cArea->moutbuffer->length (), 1),
            0);
    }
    else
    {
...
void GMessageArea::resize (int X, int Y, int W, int H)
{
    ...
    if (mtype == 1)
    {
        ...
        gnames->resize (x () + (60 * scale), y () + (28 * scale),
                        w () - (70 * scale), (32 * scale));
