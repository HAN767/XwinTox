...
TOXDNS3::TOXDNS3(XWF_ObjectParams_t *prmObj)
: Tox3Server_cnt(3)
{
...
void TOXDNS3::filterSignal(unsigned int, PBMessage_t* msg)
{
	...
	char *name, *domain =msg->S1, *tofree =msg->S1, newdomain[255], txt[1024];
	const char *rd;
	...
	{
		if(strcmp(Tox3Servers[i].name, domain) == 0) server =i;
	}
	...
	if(ns_initparse(response, response_len, &nsmsg) < 0) goto fail;

	rrmax =ns_msg_count(nsmsg, ns_s_an);

	if(ns_parserr(&nsmsg, ns_s_an, 0, &rr)) goto fail;
