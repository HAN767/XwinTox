...
EXPORT
{
    ...
    FL_EXPORT_C (void *, Fl_Input_user_data) (fl_Input input)
    {
        C_to_Fl_Callback * stored_cb = (static_cast<C_to_Fl_Callback *> (
            (static_cast<Fl_Input *> (input))->user_data ()));
        if (stored_cb)
        {
    ...
    {
        return (fl_Window) (static_cast<Fl_Input *> (input))
    ...
    {
        return (fl_Gl_Window) (static_cast<Fl_Input *> (input))
            ->as_gl_window ();
    }
