...
DerivedText_Editor::Key_Binding_With_Callback *
    DerivedText_Editor::global_key_bindings = 0;
#endif

DerivedText_Editor::DerivedText_Editor (int x, int y, int w, int h,
                                        const char * t)
    : Fl_Text_Editor (x, y, w, h, t)
{
    key_bindings              = 0;
...
void DerivedText_Editor::replace_key_bindings (Key_Binding_With_Callback ** l1,
                                               Key_Binding_With_Callback ** l2)
{
...
void DerivedText_Editor::add_key_binding (int key, int state,
...
DerivedText_Editor::Key_Binding_With_Callback *
DerivedText_Editor::convert_keybindings (Key_Binding * bindings)
{
    ...
    for (; curr_super; curr_super = curr_super->next)
    {
        ...
        C_to_Fl_Callback * cb_object =
            new C_to_Fl_Callback (&(curr_super->function));
...
C_to_Fl_Callback *
DerivedText_Editor::bound_key_function (int key, int state,
                                        Key_Binding_With_Callback * list)
{
    Key_Binding_With_Callback * curr = list;
                                             ...
                                             Key_Binding_With_Callback ** list)
{
    ...
    {
        if (curr->key == key && curr->state == state)
            break;
    }
    if (!curr)
        return;
...
int DerivedText_Editor::handle_key ()
{
    ...
    int key = Fl::event_key (), state = Fl::event_state (),
        c = Fl::event_text ()[0];
    state &=
        FL_SHIFT | FL_CTRL | FL_ALT | FL_META; // only care about these states
    C_to_Fl_Callback * f;
    f                           = bound_key_function (key, state, global_key_bindings);
    this->curr_callback_context = f;
    if (!f)
        f = bound_key_function (key, state, key_bindings);
    if (f)
        return C_to_Fl_Callback::intercept (key, this);
