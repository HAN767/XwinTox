...
#ifdef __cplusplus
DerivedText_Editor::Key_Binding_With_Callback *
convertKeyBindings (Key_BindingC * bindings)
{
    Key_BindingC * curr = bindings;
    DerivedText_Editor::Key_Binding_With_Callback * start =
        new DerivedText_Editor::Key_Binding_With_Callback ();
    ...
    for (; curr; curr = curr->next, ret_curr = ret_curr->next)
    {
        if (curr)
        {
            ...
            C_to_Fl_Callback * callback_context =
                new C_to_Fl_Callback (bindings->function);
...
Key_BindingC *
convertKeyBindings (DerivedText_Editor::Key_Binding_With_Callback * bindings)
{
...
EXPORT
{
    ...
    {
        (static_cast<DerivedText_Editor *> (win))
            ->parent ((static_cast<DerivedText_Editor *> (grp)));
    }
    ...
    FL_EXPORT_C (void *, Fl_Text_Editor_user_data) (fl_Text_Editor win)
    {
        C_to_Fl_Callback * stored_cb = (static_cast<C_to_Fl_Callback *> (
            (static_cast<Fl_Text_Editor *> (win))->user_data ()));
        if (stored_cb)
        {
    ...
    {
        return (static_cast<DerivedText_Editor *> (win))
    ...
    {
        return (fl_Window) (static_cast<DerivedText_Editor *> (text_editor))
            ->window ();
    }
    ...
    {
        return (fl_Text_Editor) (static_cast<DerivedText_Editor *> (win))
            ->as_group ();
    }
    FL_EXPORT_C (fl_Gl_Window, Fl_Text_Editor_as_gl_window) (fl_Text_Editor win)
    {
        return (fl_Gl_Window) (static_cast<DerivedText_Editor *> (win))
            ->as_gl_window ();
    }
    ...
    {
        return (fl_Widget) (static_cast<DerivedText_Editor *> (win))
            ->resizable ();
    }
    ...
    {
        return (fl_Widget *)(static_cast<DerivedText_Editor *> (self))
            ->array ();
    }
    ...
    {
        return (static_cast<DerivedText_Editor *> (text_editor))
            ->buffer ((static_cast<Fl_Text_Buffer *> (buf)));
    }
    ...
    {
        (static_cast<DerivedText_Editor *> (text_editor))
    ...
    FL_EXPORT_C(int,Fl_Text_Editor_count_lines)(fl_Text_Editor text_editor,int start,int end,int start_pos_is_line_start)
    {
        return (static_cast<DerivedText_Editor *> (text_editor))
            ->count_lines (start, end,
                           start_pos_is_line_start >= 1 ? true : false);
    }
    ...
    {
        Fl_Text_Editor::Style_Table_Entry * s =
            new Fl_Text_Editor::Style_Table_Entry[nStyles]();
        int i = 0;
        for (; i < nStyles; i++)
        {
            Fl_Text_Editor::Style_Table_Entry x =
        ...
        (static_cast<DerivedText_Editor *> (text_editor))
            ->highlight_data ((static_cast<Fl_Text_Buffer *> (styleBuffer)), s,
                              nStyles, unfinishedStyle, unfinishedHighlightCB,
                              cbArg);
    }
    FL_EXPORT_C(int,Fl_Text_Editor_position_style)(fl_Text_Editor text_editor,int lineStartPos,int lineLen,int lineIndex)
    {
        return (static_cast<DerivedText_Editor *> (text_editor))
            ->position_style (lineStartPos, lineLen, lineIndex);
    }
    ...
    {
        DerivedText_Editor::Key_Binding_With_Callback * bs =
            convertKeyBindings (list);
        DerivedText_Editor * e =
            (static_cast<DerivedText_Editor *> (text_editor));
    ...
    FL_EXPORT_C(void,Fl_Text_Editor_add_key_binding)(fl_Text_Editor text_editor,int key,int state,fl_Key_Func f)
    {
        C_to_Fl_Callback * context = new C_to_Fl_Callback (f);
        (static_cast<DerivedText_Editor *> (text_editor))
            ->add_key_binding (key, state, context);
    }
