...
EXPORT
{
    ...
    FL_EXPORT_C (void *, Fl_Clock_user_data) (fl_Clock b)
    {
        C_to_Fl_Callback * stored_cb = (static_cast<C_to_Fl_Callback *> (
            (static_cast<Fl_Clock *> (b))->user_data ()));
        if (stored_cb)
        {
    ...
    {
        return (fl_Window) (static_cast<Fl_Clock *> (clock))
    ...
    {
        return (fl_Gl_Window) (static_cast<Fl_Clock *> (clock))
            ->as_gl_window ();
    }
