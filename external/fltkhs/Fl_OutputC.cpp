...
EXPORT
{
    ...
    FL_EXPORT_C (void *, Fl_Output_user_data) (fl_Output output)
    {
        C_to_Fl_Callback * stored_cb = (static_cast<C_to_Fl_Callback *> (
            (static_cast<Fl_Output *> (output))->user_data ()));
        if (stored_cb)
        {
    ...
    {
        return (fl_Window) (static_cast<Fl_Output *> (output))
    ...
    {
        return (fl_Gl_Window) (static_cast<Fl_Output *> (output))
            ->as_gl_window ();
    }
