...
EXPORT
{
    ...
    FL_EXPORT_C (void *, Fl_Timer_user_data) (fl_Timer adjuster)
    {
        C_to_Fl_Callback * stored_cb = (static_cast<C_to_Fl_Callback *> (
            (static_cast<Fl_Timer *> (adjuster))->user_data ()));
        if (stored_cb)
        {
    ...
    {
        return (static_cast<Fl_Timer *> (adjuster))
    ...
    {
        return (fl_Window) (static_cast<Fl_Timer *> (adjuster))
