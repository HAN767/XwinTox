#include "Fl_ImageC.h"
#ifdef __cplusplus
Fl_DerivedImage::Fl_DerivedImage (int W, int H, int D,
                                  fl_Image_Virtual_Funcs * fs)
    : Fl_Image (W, H, D)
{
    overriddenFuncs = fs;
    other_data      = 0;
...
void * Fl_DerivedImage::get_other_data () { return this->other_data; }
void Fl_DerivedImage::set_other_data (void * data) { this->other_data = data; }
...
Fl_Image * Fl_DerivedImage::copy () { return Fl_Image::copy (); }
Fl_Image * Fl_DerivedImage::copy (int W, int H)
{
    if (overriddenFuncs->copy)
    {
        fl_Image image =
            (fl_Image) this->overriddenFuncs->copy ((fl_Image) this, W, H);
...
EXPORT
{
    ...
    {
        fl_Image_Virtual_Funcs * ptr =
            (fl_Image_Virtual_Funcs *)malloc (sizeof (fl_Image_Virtual_Funcs));
        ptr->draw          = NULL;
    ...
    {
        return (static_cast<Fl_DerivedImage *> (image))
            ->label ((static_cast<Fl_Widget *> (w)));
    }
    FL_EXPORT_C(void,Fl_Image_label_with_menu_item)(fl_Image image,fl_Menu_Item m)
    {
        return (static_cast<Fl_DerivedImage *> (image))
            ->label ((static_cast<Fl_Menu_Item *> (m)));
    }
