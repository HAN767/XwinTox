...
void ModuleManager_init (XWF_Call_f fnAppCall)
{
    ...
    /* Set the module manager version. Not presently used, but it might be some
     * day.
     */
    ...
    pmmManager->psrvServices.fnDesubscribe = ModuleManager_desubscribe_;
...
int ModuleManager_loadDynamicModule (const char * pszPath)
{
    ...
    if ((pszDLError = dlerror ()) != NULL)
        goto dlerror;

    /* Find the initialisation function. Remember, we only need to explicitly
     * look up just this one symbol; the module itself as well as other
     * functions
    ...
    if (ModuleManager_initialiseModule_ (pmodNew, fnInit) != 0)
        goto error;
...
XWF_Object_Handle_t * ModuleManager_createObject (const char * p_pszType)
{
    ...
    if ((pobjHandler = Dictionary_get_pointer (pmmManager->dictpobjObjects,
                                               pszType)) != 0)
    {
        XWF_Object_Handle_t * pobjhCreated =
            malloc (sizeof (XWF_Object_Handle_t));
...
int ModuleManager_destroyObject (XWF_Object_Handle_t * pobjhToDelete)
{
    ...
    {
        /* But through the classes own destroy function can we destroy one that
         * does. */
        pobjhToDelete->pxwoClass->fnDestroy (pobjhToDelete->hObj);
        /* Class doesn't need to have the object handle. So we delete it
         * ourselves. */
        free (pobjhToDelete);
        return 0;
    }
...
{
    /* lstClasses will be populated later by any classes that the module
     * registers
     * with the manager.
     * We don't actually need it since classes are first-class standalone
     * objects.
     * But it's useful to give a listing of classes that a specific module
     * registers
     * for whatever reason. */
    ...
    {
        /* It had some error, but none sufficient to warrant considering loading
         * the
...
int ModuleManager_registerClass_ (const XWF_Class_t * pobjRegistered)
{
    ...
    else
    {
        dbg ("Adding new %s class <%s> of subtype <%s> provided by <%s>\n",
             ...
             pobjRegistered->pmodProvider->pszName);
        /* So add it to the dictionary, which maps class names to their
         * handlers. */
...
{
    /* A service with a slash not preceded by the name of some provider is a
     * service
    ...
    if (strncmp (pszService, "/", 1) == 0)
    {
        ...
        if (strcmp (pszService, "/GETMODULESINFO") == 0)
        {
            ...
            modinfoNew->pszType =
                strdup (XWF_Modtype_Text_sz[modEntry->enModtype]);
        ...
        else if (strcmp (pszService, "/GETMODULECLASSESINFO") == 0)
        {
            ...
            asprintf (&clsiNew->pszName, "%s.%s", clsCur->pszType,
                      clsCur->pszSubtype);
            asprintf (&clsiNew->pszLang, "%s",
                      XWF_Lang_Text_sz[clsCur->enLang]);
...
int ModuleManager_desubscribe_ (const XWF_Object_Handle_t * hObject, int mtype,
                                void * custom, PB_Callback_f callback)
{
    dbg ("Object of class %s.%s desubscribed to signal %d\n",
         hObject->pxwoClass->pszType, hObject->pxwoClass->pszSubtype, mtype);
    /* Register with the global postbox the appropriate data. */
    PB_Deregister (pmmManager->pbGlobal, mtype, custom, callback);
    return 0;
}
...
int ModuleManager_registerFilter_ (const XWF_Object_Handle_t * hObject,
                                   int mtype, void * custom,
